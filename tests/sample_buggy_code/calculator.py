...
def validate_number(value, name: str = "value") -> None:
    ...
    if isinstance(value, float) and (math.isnan(value) or math.isinf(value)):
        raise InvalidInputError(f"{name} must be a finite number, got {value}")
...
class Calculator:
    ...
    def __init__(self, first_value: Number = 0, second_value: Number = 0) -> None:
        ...
        validate_numbers(first_value, second_value)
        
        # Initialize attributes
        self._result: Number = 0
        self._first_value: Number = first_value
        self._second_value: Number = second_value
